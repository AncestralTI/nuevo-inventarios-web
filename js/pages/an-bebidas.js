// js/pages/an-bebidas.js
// Renderer de la página real "An. Bebidas" del PBIX (sección 4.1 de ANALISIS_PBIX.md), con datos
// reales generados por pipeline/build-an-bebidas.mjs (data/an-bebidas.json). Es una copia literal
// de js/pages/an-vinos.js (misma layout/medidas en el PBIX original -- solo cambia el filtro de
// categoría de página y los textos de cabecera), tal como anticipaba el comentario de ese archivo.
//
// Expone window.AnBebidasPage.render(el) para que js/app.js delegue el render de panelGrid/kpiRow
// cuando la página activa es "an-bebidas".
//
// Nota de diseño -- slicer de Producto (ver ANALISIS_PBIX.md 4.1 y 4.2): en el PBIX original, el
// slicer "Producto" de esta página tiene su PROPIO filtro, más amplio que el filtro de página:
// Categoria IN {AGUAS, CERVEZA, GASEOSA, VINOS, VINOS IMPORTADOS, "5. Bebidas alcohólicas"} (el
// mismo conjunto ampliado que usa el slicer de "An. Vinos"). Aquí, igual que en an-vinos.js, el
// dropdown de Producto simplemente lista `data.productos`, que vienen acotados por el filtro DE
// PÁGINA del pipeline (categorías de esta página únicamente), no por ese filtro más amplio del
// slicer. Se decidió NO replicar el filtro ampliado del slicer (dejando el mismo comportamiento ya
// enviado y validado en "An. Vinos", por consistencia) porque el efecto práctico es mínimo y ya
// aceptado: un producto de AGUAS no tiene ninguna medida distinta de cero bajo el filtro de PÁGINA
// de Bebidas (ninguna fila de Vtas/Compras/Inventario de ese producto cae en esta página), así que
// mostrarlo en el slicer no cambiaría ningún dato visible -- solo agregaría opciones "vacías" al
// dropdown. Si se requiere fidelidad 1:1 del slicer más adelante, haría falta pasar la lista ancha
// de categorías del slicer como un segundo parámetro al pipeline (independiente de `categorias` de
// página) y exponerla en el JSON como `productosSlicer` o similar.
...
(function () {
  ...
  const DATA_URL = "data/an-bebidas.json";
  const TITULO_MODULO = "An. Bebidas";
  const FILTRO_PAGINA_TEXTO = "Categoría IN: 5. Bebidas alcohólicas, GASEOSA, CERVEZA";
    ...
    try {
      ...
      const doRefresh = async (btn) => {
        ...
        try {
          ...
          console.error("an-bebidas: error al actualizar", err);
        } finally {
      ...
      console.error("an-bebidas:", err);
    }
  }

  window.AnBebidasPage = { render };
})();
